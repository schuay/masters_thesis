...
Note that this operation is $O(1)$ for each block since the minimal item is located at the head of
head array.

Both insertions and deletions are of amortized complexity $O(\log n)$.
