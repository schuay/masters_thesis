...
has been taken (removed) from the \ac{DLSM} or not, with the \ac{LSM} storing pointers to these items.
...
an atomic \ac{CAS} instruction. Special care is taken in order to remove physically taken items
...
and thus we cannot ignore these issues entirely. The set of blocks in the \ac{LSM} is modelled by
...
appended atomically using a \ac{CAS} instruction. While other threads currently reading the local list
