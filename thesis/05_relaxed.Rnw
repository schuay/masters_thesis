...
by up to a factor of two, but only scales while executed on a single socket.
...
collisions between multiple concurrent \lstinline|delete_min| calls are unlikely. This is achieved by
...
of \lstinline|delete_min|, which the authors consider as the performance ideal (although it might be
...
% TODO: Reevaluate the bound guarantees. Is (k+1)P really the best we can provide?
...
$k$ items through relaxation of the \ac{SLSM}. We therefore still stay comfortably
