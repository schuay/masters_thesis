...
framework or non-standard library and is engineered to be easily integrated into
...
threads are either dedicated inserters or deleters) and ascending key generation
(in which the value of generated item keys increases over time).

As demonstrated previously \cite{wimmer2015lock}, the \klsm scales exceptionally
...
The \klsm with $k = 4096$ shows linear speedup up to the maximal number of threads
on all test machines. This is a significant improvement over the Pheet \klsm implementation by
\citeauthor{wimmer2015lock}, which does not scale beyond 40 threads.
...
of returning one of the $(k + 1) \cdot P$ least elements; the provided results are
...
Despite the findings of this thesis, the \klsm remains an intensely interesting
...
but only below average in others. Thus the \klsm might be an ideal candidate
