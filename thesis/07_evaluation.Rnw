...
% TODO: Possibly add a short figure + description for descending key generation.
...
% TODO: Really? Should we include the sssp benchmark?
\begin{comment}
...
% TODO: At least briefly mention our use of the specialized tree and post-processing
% in contrast to Multiqueues' naive multiset + disruptive interweaved processing.
...
utilizing more than one socket, while the Multiqueues barely scale at all.
On \lstinline|ceres| on the other hand, even $k = 128$ seems to be able
...
\klsm may return any of the $(k + 1) \cdot P$ minimal elements, where $P$ is the number
...
a lower initial peak at one socket, and a more pronounced oscillating behavior
of the \klsm.
...
keygeneration (see Table \ref{tbl:mars_uni_asc_q}). On the other hand, the
...
\klsm ($k + 1$ instead of $(k + 1) \cdot P$), the resulting rank errors are low.
...
especially detrimental for the \klsm; and as expected all \klsm variants
do not perform well, never exceeding 5 \ac{MOps} on any machine and with any
thread count.
...
behavior while within a single socket ($\leq 10$ threads), with high mean rank
