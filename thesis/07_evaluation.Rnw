...
replayed using a strict sequential queue\footnote{}, and ranks of each deleted item
...
The strict priority queue used within the quality benchmark supports an operation which
deletes a specific element and its rank. Contrary to similar benchmarks by
\citeauthor{rihani2014multiqueues} \cite{rihani2014multiqueues}, we use a custom
