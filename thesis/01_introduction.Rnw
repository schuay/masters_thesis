...
are used as building blocks in a large variety
of situations such as shortest path algorithms and scheduling.
In their most basic form, priority queues  consisting of two operations
...
of threads. Multiqueues \cite{rihani2014multiqueues} have a simple and elegant design, which uses a number
