...
This thesis focuses on the priority queue data structure, consisting of two operations
...
a complexity of at most $O(\log n)$. Priority queues are essential data structures and
are used as basic building blocks in a large variety
...
strictness of provided guarantees against improved scalability. The SprayList \cite{alistarhspraylist} is
...
of threads. Multiqueues \cite{rihani2014multiqueues} are a simple and elegant design, which uses a number
...
The \klsm in particular has shown very high scalability; but since it
...
state-of-the-art priority queues have been difficult so far. During the course of
this thesis, we have implemented a standalone \klsm variant in order to verify the findings of
\cite{wimmer2015lock} outside the context of the Pheet framework.

% Benching.

Recent literature on concurrent priority queues
\cite{hunt1996efficient,alistarhspraylist,linden2013skiplist,shavit2000skiplist,sundell2003fast,wimmer2015lock,cbpq}
has overwhelmingly relied on a uniform workload, uniform key generation throughput benchmark for evaluation,
in which all threads perform a roughly equal mix of insertions and deletions,
and item keys are generated uniformly at random within some domain. It is, however,
...
Consequently, we extended our benchmarks with two parameters: key generation
may be either uniformly at random, descending (for \ac{LIFO}-like behavior)
or ascending (\ac{FIFO}-like). Workload may be either uniform, or split (in which
...
benchmark, outperforming the best other queue by almost a factor of ten. However,
its throughput drops rapidly rapidly in other cases.
...
item is one of the $(k + 1) \cdot P$ minimal elements, it would also be useful
...
measurements on the \klsm and other compatible priority queues.
