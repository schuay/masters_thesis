...
\begin{itemize}
\justifying
\item \emph{Priority queues}: Abstract data structures which store a set of key/value pairs;
efficient access to the item with the minimal (maximal) key.

\item \emph{Concurrent priority queues}: Developed from early locking heaps,
through lock-free SkipList-based designs, to modern research into relaxed priority queues.

\item \emph{Lock-freedom}: A strong progress condition, has been shown to be
efficient in practice.

\item \emph{Relaxation}: An approach which weakens semantic guarantees in return for
...
\item \emph{Performant}: Outperforms the best other concurrent
priority queues by up to a factor of ten.

\item \emph{Maintainable}: Implemented using classical software
engineering concepts such as composability, readability, and separation of concerns.

\item \emph{Portable}: Implemented using C++11 atomics without reliance on external libraries.
\end{itemize}
\end{block}
...
\begin{itemize}
\justifying
\item \emph{Algorithms}: Evaluated against other state of the art designs, including
...
Rank Error & Mean & Std. Dev. & Mean & Std. Dev & Mean & Std. Dev \\
\midrule%
